...
int main(){
    ...
    loadMatrix("maps/B3000.txt", matrix, width, height);
    QuadTree dumir(matrix, width, height);

    /*
    Node *n{dumir.FindNode(Point(200,200))};
    std::cout << n->topLeft.x << ", " << n->topLeft.y << "; " << n->botRight.x << ", " << n->botRight.y << std::endl;
    for(int i{}; i<50; i++){
        Point p{n->RandomPoint()};
        std::cout << p.x << "," << p.y << std::endl;
    }
    */
    Astar astar(&dumir);
    //astar.CreateFullMatlabPlot("instructions.m", Point(200, 200), Point(500, 2500));
    //astar.CreateFullMatlabPlotRRT("instructions.m", Pose(200, 200, 2*atan(1)), Pose(500, 2500, 4*atan(1)));
    //astar.CreateFullMatlabPlotRRT("instructions.m", Pose(200, 200, 2*atan(1)), Pose(2500, 500, 4*atan(1)));
    //astar.CreateFullMatlabPlotRRT("instructions.m", Pose(50, 50, 2*atan(1)), Pose(550, 50, 4*atan(1)));
    //astar.CreateFullMatlabPlotRRT("instructions.m", Pose(5, 5, 2*atan(1)), Pose(95, 5, 3*4*atan(1)));
    astar.CreateFullMatlabPlotRRT("instructions.m", Pose(500, 2700, 3*2*atan(1)), Pose(2700, 200, 3*4*atan(1)));
    delete[] matrix;
    return 0;
}
