...
public:
    ...
    explicit Astar(QuadTree* quadtree) : quadtree(quadtree){}
    ...
    void CreateFullMatlabPlotRRT(const std::string &path, Pose start, Pose finish);
...
void Astar::CreateFullMatlabPlotRRT(const std::string &path, Pose start, Pose finish){
    ...
    std::vector<Node*> nodes{FindPath(start, finish)};
    RRT rrt(quadtree);
    std::vector<Pose> trajectory{rrt.RefinePath(nodes, start, finish)};
